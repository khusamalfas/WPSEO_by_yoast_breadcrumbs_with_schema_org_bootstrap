<?php
   ...
   */
  class custom_WPSEO_breadcrumbs_Schema_bootstrap extends WPSEO_Breadcrumbs
  {
    ...
    private function crumb_to_link($link, $i)
    {
      ...
      if (isset($link['text']) && (is_string($link['text']) && $link['text'] !== '')) {
        ...
        $count = $i + 1;
        if ((isset($link['url']) && (is_string($link['url']) && $link['url'] !== '')) && ($i < ($this->crumb_count - 1))
        ) {
          if ($i === 0) {
            $link_output .= '<li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">';
          } else {
            $link_output .= '<li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">';
          }
          $title_attr   = isset($link['title']) ? ' title="' . esc_attr($link['title']) . '"' : '';
          $link_output .= '<a itemtype="https://schema.org/Thing" itemscope itemprop="item" itemid="' . esc_url($link['url']) . '" href="' . esc_url($link['url']) . '" ' . $title_attr . '><span itemprop="name">' . $link['text'] . '</span></a>';
          $link_output .= '<meta itemprop="position" content="' . $count . '">';
          $link_output .=  '</li>';
        } else {
          $link_output .= '<li class="breadcrumb-item active" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">';
          $title_attr   = isset($link['title']) ? ' title="' . esc_attr($link['title']) . '"' : '';
          $link_output .= '<a aria-current="page" class="current" itemtype="https://schema.org/Thing" itemscope itemprop="item" itemid="' . esc_url($link['url']) . '" href="' . esc_url($link['url']) . '" ' . $title_attr . '><span itemprop="name">' . $link['text'] . '</span></a>';
          $link_output .= '<meta itemprop="position" content="' . $count . '">';
          $link_output .=  '</li>';
    ...
    private function wrap_breadcrumb()
    {
      if (is_string($this->output) && $this->output !== '') {
        $output = '<ul class="breadcrumb" id="breadcrumb-ul"  itemprop="Breadcrumb" itemscope="" itemtype="http://schema.org/BreadcrumbList" itemid="#breadcrumb-ul">' . $this->output . '</ul>';
